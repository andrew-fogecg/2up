...
</svg>`,ce=`<svg viewBox="0 0 500 120" xmlns="http://www.w3.org/2000/svg">
...
</svg>`;function de(n,e){const t=Array.from({length:5},(i,s)=>`<span class="skull-icon" data-index="${s}">${le}</span>`).join("");return`
<div class="game-root">
  ...
  <div class="ship-silhouette">${ce}</div>
              ...
              <div class="coins-on-kip">
                <div class="coin" id="coin-1">
                  <div class="coin-inner" id="coin-1-inner">
                    <div class="coin-face coin-face--front">${q}</div>
...
</div>`}const z=re(),A=z.currency||"GC",Z=z.balance??1e3,h=new ae({currency:A,startingBalance:Z}),ue=document.getElementById("app");ue.innerHTML=de(A,Z);const u=n=>document.getElementById(n),r={balance:u("balance-value"),sessionDisp:u("session-display"),playBtn:u("play-btn"),stakeInput:u("stake-input"),potentialWin:u("potential-win-value"),betSelector:u("bet-type-selector"),skullsRow:u("skulls-row"),oddsCount:u("odds-count-display"),resultBanner:u("result-banner"),resultText:u("result-text"),captainBubble:u("captain-bubble"),parrotBubble:u("parrot-bubble"),captain:u("captain"),historyList:u("history-list"),chestLeft:u("chest-left"),chestRight:u("chest-right"),screenFlash:u("screen-flash"),serverSeedHash:u("server-seed-hash"),clientSeedInput:u("client-seed-input"),rotateSeedBtn:u("rotate-seed-btn"),errorMsg:u("error-message"),goldDust:u("gold-dust"),tossCounter:u("toss-counter"),tossNum:u("toss-num"),coin1Inner:u("coin-1-inner"),coin2Inner:u("coin-2-inner"),coin1Shadow:u("coin-1-shadow"),coin2Shadow:u("coin-2-shadow")};let O=m.HEADS,S=!1,w=[],b=null;r.sessionDisp.textContent=`Session: ${h.sessionID}`;r.clientSeedInput.value=h.clientSeed;const he=setInterval(()=>{h.serverSeedHash&&(r.serverSeedHash.textContent=h.serverSeedHash.slice(0,20)+"...",clearInterval(he))},100);h.onCoinToss=(n,e,t,i)=>w.push({c1:n,c2:e,result:t,idx:i});h.onWin=(n,e)=>{b={type:"win",profit:n,balance:e}};h.onLoss=(n,e)=>{b={type:"loss",amount:n,balance:e}};h.onOddsStreak=()=>{};h.onOddedOut=n=>{b?(b.oddedOut=!0,b.balance=n):b={type:"loss",amount:0,balance:n,oddedOut:!0}};h.onStateChange=()=>{};h.onError=n=>G(n);(function(){for(let e=0;e<30;e++){const t=document.createElement("div"),i=(Math.random()-.4)*70;t.className="gold-particle",t.style.cssText=[`left:${15+Math.random()*70}%`,`bottom:${3+Math.random()*38}%`,`animation-delay:${(Math.random()*5).toFixed(2)}s`,`animation-duration:${(3.5+Math.random()*4).toFixed(2)}s`,`width:${(2+Math.random()*3).toFixed(1)}px`,`height:${(2+Math.random()*3).toFixed(1)}px`,`--drift:${i.toFixed(0)}px`].join(";"),r.goldDust.appendChild(t)}})();r.betSelector.addEventListener("click",n=>{const e=n.target.closest(".bet-btn");!e||S||(r.betSelector.querySelectorAll(".bet-btn").forEach(t=>t.classList.remove("active")),e.classList.add("active"),O=e.dataset.bet,I())});r.stakeInput.addEventListener("input",I);document.querySelectorAll(".quick-btn").forEach(n=>{n.addEventListener("click",()=>{S||(r.stakeInput.value=n.dataset.stake==="max"?String(Math.floor(h.balance/1e6)):n.dataset.stake,I())})});function I(){const n=parseFloat(r.stakeInput.value)||0,e=O===m.FIVE_ODDS?28:1;r.potentialWin.textContent=`${(n*e).toLocaleString()} ${A}`}r.clientSeedInput.addEventListener("change",()=>h.setClientSeed(r.clientSeedInput.value));r.rotateSeedBtn.addEventListener("click",async()=>{if(S)return;const n=await h.rotateServerSeed();n&&(r.serverSeedHash.textContent=n.newHash.slice(0,20)+"...",r.clientSeedInput.value=h.clientSeed,g("Seeds rotated! New round begins fresh, matey!",2500))});function R(n){const e=Math.floor(n/1e6);r.balance.textContent=`${e.toLocaleString()} ${A}`}function U(n){r.skullsRow.querySelectorAll(".skull-icon").forEach((e,t)=>{e.classList.toggle("active",t<n),e.classList.toggle("danger",n>=4&&t<n)}),r.oddsCount.textContent=`${n} of 5`,r.oddsCount.classList.toggle("danger",n>=3)}let P=null;function g(n,e=3200){r.captainBubble.textContent=n,r.captainBubble.classList.add("visible"),clearTimeout(P),P=setTimeout(()=>r.captainBubble.classList.remove("visible"),e)}function x(n){r.captain.className=`captain captain--${n}`}let Y=null;function E(n,e=2800){n&&(r.parrotBubble.textContent=n,r.parrotBubble.classList.add("visible"),clearTimeout(Y),Y=setTimeout(()=>r.parrotBubble.classList.remove("visible"),e))}let Q=null;function G(n){r.errorMsg.textContent=n,r.errorMsg.classList.add("visible"),clearTimeout(Q),Q=setTimeout(()=>r.errorMsg.classList.remove("visible"),3200)}function D(n,e,t=2200){return r.resultText.textContent=n,r.resultBanner.className=`result-banner visible result--${e}`,B(t).then(()=>{r.resultBanner.className="result-banner"})}function pe(){const n=document.querySelector(".game-root");r.screenFlash.classList.remove("flash-red"),r.screenFlash.offsetWidth,r.screenFlash.classList.add("flash-red"),n.classList.remove("screen-shake"),n.offsetWidth,n.classList.add("screen-shake"),setTimeout(()=>{r.screenFlash.classList.remove("flash-red"),n.classList.remove("screen-shake")},650)}function K(){r.chestLeft.classList.add("open"),r.chestRight.classList.add("open"),setTimeout(()=>{r.chestLeft.classList.remove("open"),r.chestRight.classList.remove("open")},3200)}function B(n){return new Promise(e=>setTimeout(e,n))}async function X(n,e,t){const i=t==="H"?1800:1980;n.getAnimations().forEach(a=>a.cancel()),e.getAnimations().forEach(a=>a.cancel());const s=n.animate([{transform:"translateY(0px) rotateY(0deg) scale(1)",filter:"blur(0px)",offset:0},{transform:"translateY(-35px) rotateY(270deg) scale(1.08)",filter:"blur(0px)",offset:.07},{transform:"translateY(-280px) rotateY(900deg) scale(0.82)",filter:"blur(2.5px)",offset:.44},{transform:"translateY(-280px) rotateY(1440deg) scale(0.82)",filter:"blur(3px)",offset:.56},{transform:"translateY(-35px) rotateY(1700deg) scale(1.06)",filter:"blur(1px)",offset:.87},{transform:`translateY(0px) rotateY(${i}deg) scale(1)`,filter:"blur(0px)",offset:1}],{duration:1850,easing:"cubic-bezier(0.25,0.08,0.25,1)",fill:"forwards"});e.animate([{transform:"translateX(-50%) scaleX(1)",opacity:.45,offset:0},{transform:"translateX(-50%) scaleX(0.25)",opacity:.08,offset:.44},{transform:"translateX(-50%) scaleX(0.25)",opacity:.08,offset:.56},{transform:"translateX(-50%) scaleX(1)",opacity:.45,offset:1}],{duration:1850,easing:"cubic-bezier(0.25,0.08,0.25,1)",fill:"forwards"}),await s.finished}async function me(n){const{c1:e,c2:t,result:i,idx:s}=n;r.tossCounter.style.display="block",r.tossNum.textContent=String(s),y.coinLaunch(),await Promise.all([X(r.coin1Inner,r.coin1Shadow,e),X(r.coin2Inner,r.coin2Shadow,t)]),await B(380),i==="ODDS"?(y.oddsResult(),await D("ODDS!","odds",1300),g(f(T.ODDS),2200),E(f(_.ODDS),2200),[r.coin1Inner,r.coin2Inner,r.coin1Shadow,r.coin2Shadow].forEach(a=>a.getAnimations().forEach(o=>o.cancel())),await B(700)):i==="HEADS"?y.headsLand():y.tailsLand()}function fe(n){const e=r.historyList.querySelector(".history-empty");e&&e.remove();const{round:t,betType:i,betAmount:s,tosses:a,outcome:o,profit:l,balance:c}=n,d=Math.floor(s/1e6),p=Math.floor(l/1e6),J=Math.floor(c/1e6),ee=p>=0?`+${p}`:String(p),te=p>=0?"profit-positive":"profit-negative",se=o==="HEADS"?"outcome-heads":o==="TAILS"?"outcome-tails":o==="ODDED_OUT"?"outcome-odded":"outcome-odds",L=document.createElement("div");L.className="history-entry",L.innerHTML=`
    <span class="history-round">#${t}</span>
    <span class="history-bet">${i} ${d} ${A}</span>
    <span class="history-tosses">${a.join(" → ")}</span>
    <span class="history-outcome ${se}">${o}</span>
    <span class="history-profit ${te}">${ee} ${A}</span>
    <span class="history-balance">${J.toLocaleString()} ${A}</span>
  `,r.historyList.insertBefore(L,r.historyList.firstChild),requestAnimationFrame(()=>requestAnimationFrame(()=>L.classList.add("history-entry--visible")));const V=r.historyList.querySelectorAll(".history-entry");V.length>10&&V[V.length-1].remove()}async function j(){if(S)return;const n=parseFloat(r.stakeInput.value);if(!n||n<=0){G("Enter a valid wager, ye scoundrel!");return}const e=Math.round(n*1e6);if(e>h.balance){G("Insufficient doubloons! Ye cannot bet what ye don't have!");return}w=[],b=null,R(h.balance-e),S=!0,r.playBtn.disabled=!0,r.playBtn.classList.add("spinning"),r.errorMsg.classList.remove("visible"),r.tossCounter.style.display="none",U(0),g("Come in, Spinner! The coins are in the air!",2800),x("neutral"),await h.startRound(O,n);let t=0;for(const i of w)i.result==="ODDS"&&(t++,U(t)),await me(i);if(R(h.balance),b){const i=w[w.length-1],s=b.oddedOut===!0,a=s&&O===m.FIVE_ODDS,o=b.type==="win",l=o&&b.profit>=e*4;if(a)y.bigWin(),await D("FIVE ODDS! JACKPOT!","heads",3200),x("excited"),g(f(T.ODDED_OUT),4200),E(f(_.ODDED_OUT),3500),K();else if(s)y.oddedOut(),await D("ODDED OUT! CURSED!","odded",3200),x("angry"),g(f(T.ODDED_OUT),4200),E(f(_.ODDED_OUT),3500),pe();else if(o){l?(y.bigWin(),K()):y.headsLand();const c=i?i.result:"WIN";await D(`${c}! YE WIN!`,"heads",2800),x("excited"),g(f(T.WIN),3800),E(f(_.WIN),3200)}else{y.lose();const c=i?i.result:"LOSS";await D(`${c}! YE LOSE!`,"tails",2500),x("angry"),g(f(T.LOSS),3800),E(f(_.LOSS),3200)}}h.roundHistory.length>0&&fe(h.roundHistory[0]),r.tossCounter.style.display="none",S=!1,r.playBtn.disabled=!1,r.playBtn.classList.remove("spinning"),[r.coin1Inner,r.coin2Inner,r.coin1Shadow,r.coin2Shadow].forEach(i=>i.getAnimations().forEach(s=>s.cancel()))}r.playBtn.addEventListener("click",()=>{y.buttonClick(),j()});r.betSelector.addEventListener("click",()=>y.uiHover(),{capture:!0});document.addEventListener("keydown",n=>{n.code==="Space"&&!n.repeat&&(n.preventDefault(),S||j())});I();R(h.balance);x("neutral");g(f(T.IDLE),3500);setInterval(()=>{S||g(f(T.IDLE),3500)},9e3);
